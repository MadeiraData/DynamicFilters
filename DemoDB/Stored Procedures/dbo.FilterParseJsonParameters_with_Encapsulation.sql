...
EXEC dbo.[FilterParseJsonParameters_with_Encapsulation] @SourceTableAlias = 'Members', @JsonParams = @JsonParams, @JsonOrdering = @JsonOrdering
		, @ParsedSQL = @SQL OUTPUT, @CMD = @CMD OUTPUT, @CMDParams = @CMDParams OUTPUT

PRINT @SQL

EXEC sp_executesql @SQL, N'@JsonParams NVARCHAR(MAX), @CMD NVARCHAR(MAX), @CMDParams NVARCHAR(MAX)', @JsonParams, @CMD, @CMDParams

*/
CREATE PROCEDURE [dbo].[FilterParseJsonParameters_with_Encapsulation]
...
SELECT
	  @CMDParams = ISNULL(@CMDParams + N', ', N'') + N'@p' + ParamIndex + ' ' + 
		...
		CASE WHEN FilterOperators.IsMultiValue = 1 THEN N'NVARCHAR(MAX)'
...
IF @RunCommand = 1
	EXEC sp_executesql @ParsedSQL, N'@JsonParams NVARCHAR(MAX), @CMD NVARCHAR(MAX), @CMDParams NVARCHAR(MAX)', @JsonParams, @CMD, @CMDParams
END